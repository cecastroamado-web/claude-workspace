...
- Catch-up (`_catchup_on_start`, ~90s após subir): daily-sales (09h) + intraday (12/15/18/21/23h) + payouts-sync (07h) + **havan-sync/briefing/ruptura (06/07/08h) + check-saldo (10h)** — os 2 últimos adicionados 14/jun/2026 (queda de DNS no host 03h-10h derrubou todas as janelas da manhã; só daily-sales tem o loop `retry-missed` separado). Mudança no `agent/scheduler.py` ainda **não commitada** (vive via restart). Havan usa guard `havan_sync_last_ok`; check-saldo é idempotente (só alerta se < mínimo, sem estado).
- **Queda de DNS = causa comum de "não recebi alertas"**: host depende só do DNS do roteador (192.168.2.1), sem FallbackDNS no systemd-resolved. Sintoma no journal: `Temporary failure in name resolution` em massa. Agente NÃO marca como enviado (retorno bool de `_notify`), então fica pendente — recupera via catch-up no restart. Diagnóstico: `journalctl -u ecommerce-agent.service | grep "name resolution"` + `getent hosts api.telegram.org`.
