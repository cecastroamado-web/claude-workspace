...
- **🔔 Alerta Telegram de estoque < mínimo** (PEDIDO CFO — A CONSTRUIR): job no scheduler (modelar em `_run_check_ima_depletion`, scheduler.py ~3667, que já baixa a planilha via Drive) que computa saldo por insumo e alerta os abaixo do mínimo pra pedir. **Computar em Python** (NÃO confiar no Estoque Atual calculado — após escrita openpyxl os valores `data_only` ficam None até abrir no Sheets). Lógica = `inicial (Saldos Iniciais+Fechamento) + entradas (Entradas col G por SKU col D) − saídas (col G por SKU col D) − CAIXAS (col J onde col N=SKU)` vs **mínimo (Parametros col D)**. A função de saldos em api.py ~3038-3170 já faz inicial+entradas−saídas por SKU, MAS falta o termo de caixa (col N/J) e o mínimo. Cuidado anti-spam (cooldown/cross-threshold).
