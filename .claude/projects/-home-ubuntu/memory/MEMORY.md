...
- [Padronização da rentabilidade Havan](ecommerce_rentabilidade_padronizacao.md) — Mark 1% no engine canônico, cabos Simples 0% crédito ICMS (jun+), painel custo (NF 586), filtro 587 no detalhe do cliente; margem Havan real ~26% (jun/2026). PENDENTE: unificar canal/PDF/simulação no engine (#11/#14)
...
- Ver [Caixa Metagraf — custo só no canal Havan](ecommerce_havan_embalagem_metagraf.md) — Havan tem caixa de papelão separada (Metagraf, NÃO Metragraf); ML mantém caixa Gedeval na base. Discriminador = SKU `HVN*` (ML=`MLB*`). Tabela `havan_embalagem` + hook no `compute_profitability` (param `havan_box_map`): CMV += embalagem+IPI−base_delta, crédito += emb×12%−base×17%, data-aware por `desde`, blindado (ML não casa). Case/Slim=todas, Haste/Painel=01/06. ICMS destacado real 12%. Planilha de insumos PENDENTE. (17/jun/2026)
- Ver [PENDENTE OC Havan 2026-55267-1 recancel](ecommerce_havan_oc_55267_recancel.md) — NF cancelada p/ refaturar não voltou a "a faturar"; 400 hastes + 150 Slim Pro seguem "em trânsito". A investigar (mesma família do detector de cancelamento de NF-e). (17/jun/2026)
- Ver [Antecipação de NFs Havan faturadas](ecommerce_havan_faturadas_antecipacao.md) — faturada sai do snapshot e vira A RECEBER no Sheets; seletor per-OC agora inclui faturadas com **data de entrega editável** (tabela `havan_faturada_entrega`, NF emitida dias antes da entrega); antecipa p/ a entrega, reflete no gráfico E na provisão, sem dupla contagem com o genérico. + **detecção automática de cancelamento de NF-e** no sync 06h30 (re-checa situacao=6 sumida da lista → confirma detalhe → `exclude_from_revenue=1` + alerta Telegram); NF 587 cancelada marcada. Ver [[ecommerce-bling-rules]]. (17/jun/2026)
...
- Ver [Em-trânsito via estoque — RESOLVIDO](ecommerce_havan_transito_estoque.md) — ✅ RESOLVIDO 18-19/jun (`3bf5f0e`+`693a44b`): `_havan_em_transito_por_sku` = faturado−recebido na JANELA ABERTA (mês corrente+anterior); timer 15d removido (const órfã). Não precisou do modelo de estoque (relatório voltou a atualizar). Fim do trânsito fantasma de meses fechados.
...
- Ver [Briefing matinal CFO — IMPLEMENTADO](ecommerce_briefing_matinal_cfo.md) — "☀️ Bom dia, CFO" único no Telegram às 06h30 pós-coleta (`scheduler._run_briefing_matinal`, loop `briefing-matinal`): Caixa hoje, Vendas, Havan, Impostos, Saúde de dados. Pula domingo/feriado + guard `briefing_matinal_last`. **Disparo manual: `_run_briefing_matinal(force=True)`** (ignora guards, não marca estado) — commit `aa9a948` (21/jun). Validado chegando certinho.
