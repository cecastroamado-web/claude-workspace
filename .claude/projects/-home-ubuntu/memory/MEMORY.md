...
- [Dashboard do canal Havan](ecommerce_havan_dashboard.md) — scraper 1×/dia 06h, margem/P&L, conciliação+em-trânsito, KPIs do PDF nos cards, analytics de expansão (rede 196 lojas: penetração, ruptura, demanda 120d A/B, vendas por filial mensal + 3 PDFs: filiais, ranking c/ período, potencial heatmap+resumo 3,4x). Pendências em [[ecommerce-havan-pendencias]].
...
- Ver [Faturamento Havan em trânsito](ecommerce_havan_backlog_faturamento_transito.md) — agregado mês a mês (`6127d17`) + **POR PRODUTO em itens** (`7ad4de9`): em-trânsito = itens de NF não entregue no CD (data_emissao+15d>hoje), tabela `havan_nfe_items`. **Caveat: snapshot `recebido` SUBCONTA** (não usar faturado−recebido). Coluna "A faturar" = pedidos_abertos−trânsito (`ce88c49`)
