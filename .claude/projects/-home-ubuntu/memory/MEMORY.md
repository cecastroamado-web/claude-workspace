...
- [Dashboard do canal Havan](ecommerce_havan_dashboard.md) — scraper 2×/dia (7h+18h, alerta só de manhã) + **5 features jun/2026**: (1) histórico de snapshots `/api/havan/history`, (2) receita Havan no fluxo de caixa (`receita_havan_est`, régua entrega+120d), (3) margem+P&L do canal, (4) previsor de devolução/sobre-estoque, (5) conciliação mês a mês + "em trânsito". Também: reposição 120d (híbrido, lead 45d, serviço 95%), preço atacado por keyword. Pendências em [[ecommerce-havan-pendencias]].
...
- Ver [Faturamento Havan em trânsito — RESOLVIDO](ecommerce_havan_backlog_faturamento_transito.md) — **resolvido** por `6127d17` (em-trânsito agregado mês a mês = faturado−recebido; pagamento entrega+120d). CFO: agregado basta. Sobra acompanhar entrega real 09-10/jun. NFs jun em trânsito = SLIM/HASTE/cabos (não cases)
